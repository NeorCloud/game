...
class UserController
{
     ...
     * @return \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
     */
    public function avatar(EditUserRequest $request, User $user)
    {
        return view('backend.auth.user.avatar', compact('user'));
    }

    public function uploadAvatar(ChangeAvatarRequest $request, User $user)
    {
        if ($request->has('avatar_location') && $request->file('avatar_location') != null) {
            $location = $request->file('avatar_location')->store('/avatars', 'public');
        } else {
            $location = null;
        }
        $this->userService->updateAvatar($user, $request->validated(), $location);

        return redirect()->back()->withFlashSuccess(__('Avatar updated successfully'));
    }
}
