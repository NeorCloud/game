    ...
    <x-backend.card>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($user->last_login_at)
                                {{verta($user->last_login_at)->formatJalaliDatetime()}}
                            @else
                                @lang('N/A')
                            @endif
                        </td>
        ...
        <x-slot name="footer">
            <small class="float-right text-muted">
                <strong>@lang('Account Created'):</strong>
                {{verta($user->created_at)->formatJalaliDatetime()}}
                ({{ $user->created_at->diffForHumans() }}),
                <strong>@lang('Last Updated'):</strong>
                {{verta($user->updated_at)->formatJalaliDatetime()}}
                ({{ $user->updated_at->diffForHumans() }})

                @if($user->trashed())
                    <strong>@lang('Account Deleted'):</strong>
                    {{verta($user->deleted_at)->formatJalaliDatetime()}}
